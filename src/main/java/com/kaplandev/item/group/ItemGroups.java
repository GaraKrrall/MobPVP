...
public class ItemGroups {
    ...
    static {
        ...
        EGGS_GROUP_KEY = RegistryKey.of(Registries.ITEM_GROUP.getKey(), Identifier.of(MOD_ID, TAB_MOBPVP_EGGS_KEY));
        MOBPVP_GROUP = Registry.register(Registries.ITEM_GROUP, MOBPVP_GROUP_KEY, FabricItemGroup.builder().icon(() -> new ItemStack(Items.KALP_ITEM)).displayName(Text.translatable("itemGroup.mobpvp")).entries((ctx, entries) -> {}).build());
        EGGS_GROUP = Registry.register(Registries.ITEM_GROUP, EGGS_GROUP_KEY, FabricItemGroup.builder().icon(() -> new ItemStack(Items.EGG_SKELETON)).displayName(Text.translatable("itemGroup.eggs")).entries((ctx, entries) -> {}).build());
    }
}
