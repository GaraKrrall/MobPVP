...
import com.kaplandev.event.totem.machine.recipe.IndustrialOvenRecipes;
...
public class INDUSTRIAL_OVEN {
    ...
    public static ActionResult onUseBlock(PlayerEntity player, World world, Hand hand, BlockHitResult hit) {
        ...
        if (chest == null) return ActionResult.PASS;
        ...
        if (recipe == null) {
            ...
            player.sendMessage(Text.literal("No valid recipe found."), false);
...
// Particle effect during process
        ...
        ServerScheduler.schedule(duration, () -> {
            ...
            if (!hasSpace) {
                ...
                player.sendMessage(Text.literal("The oven did not run because the chest is full!"), false);
                return;
            }
            ...
            if (!added && !result.isEmpty()) {
                ...
                player.sendMessage(Text.literal("The chest was full, product dropped on the ground."), false);
            }
