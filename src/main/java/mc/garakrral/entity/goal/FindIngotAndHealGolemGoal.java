                ...
                if (usingChest) {
                    ...
                    if (miniGolem.getBlockPos().isWithinDistance(targetChest, 1.5)) {
                        ...
                        if (be instanceof ChestBlockEntity chest) {
                            ...
                            for (int i = 0; i < inv.size(); i++) {
                                ...
                                if (stack.isOf(ItemType.REINFORCED_COPPER_INGOT) || stack.isOf(Items.IRON_INGOT)) {
                    ...
                    if (targetIngotEntity != null && targetIngotEntity.isAlive()) {
                        ...
                        if (miniGolem.squaredDistanceTo(targetIngotEntity) < 1) {
                            ...
                            Item selected = stack.getItem();
                            if (!stack.isEmpty() && (selected == Items.IRON_INGOT || selected == ItemType.REINFORCED_COPPER_INGOT)) {
                                if (stack.getCount() > 1) {
                                    stack.decrement(1);
            ...
            case HEAL -> {
                if (targetGolem != null && targetGolem.isAlive()) {
                    ...
                    if (targetGolem.getHealth() < targetGolem.getMaxHealth()) {
                        ...
                        if (handStack.isOf(ItemType.REINFORCED_COPPER_INGOT)) {
    ...
    private ItemEntity findNearbyIngot() {
        List<ItemEntity> items = miniGolem.getWorld().getEntitiesByClass(ItemEntity.class,
                ...
                e -> {
                    ...
                    return i == ItemType.REINFORCED_COPPER_INGOT || i == Items.IRON_INGOT;
                });

        // Öncelik: Reinforced
        return items.stream()
                .filter(e -> e.getStack().isOf(ItemType.REINFORCED_COPPER_INGOT))
                .findFirst()
